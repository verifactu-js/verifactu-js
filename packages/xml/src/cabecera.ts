...
import type { SiNo } from './registro.js';
...
export interface RemisionVoluntaria {
  /**
   * Last date the system will act as VERI*FACTU. Only when announcing a future withdrawal.
   *
   * `dd-mm-yyyy`. From 1 January 2027 the AEAT requires the shape `31-12-20XX`, and the year must
   * be the current one or the previous one.
   */
...
export interface RemisionRequerimiento {
  /** The AEAT's requirement reference, up to 18 characters. Must exist on their side. */
...
export interface Cabecera {
  ...
  /** Mutually exclusive with {@link Cabecera.RemisionRequerimiento} — see docs/spec-notes.md D-16. */
